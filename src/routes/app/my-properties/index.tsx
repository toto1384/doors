...
const fetchMyPropertiesServerFn = createServerFn()
	.validator((d) => zodValidator.parse(d))
...
function MyPropertiesRoute() {
	...
	async function fetchMyProperties({
		...
		skip?: number;
		status?: (typeof PropertyStatusValues)[number];
	}) {
	...
	const actualProperties = data.pages
		.flat(1)
		.filter((i) => searchParams.status === undefined || i.status === searchParams.status);
	...
	return (
		<div className="flex flex-col items-center justify-center border mx-2 rounded-lg">
			...
			{toDeletePropertyId && (
				<AlertDialog
					onOpenChange={(open) => {
						if (!open) setToDeletePropertyId(undefined);
					}}
					open={!!toDeletePropertyId}
				>
					<AlertDialogTrigger></AlertDialogTrigger>
					<AlertDialogContent>
						<AlertDialogHeader>
							<AlertDialogTitle>{t("my-properties.deleteDialog.title")}</AlertDialogTitle>
							<AlertDialogDescription>{t("my-properties.deleteDialog.description")}</AlertDialogDescription>
						</AlertDialogHeader>
						<AlertDialogFooter>
							...
							>
								{t("my-properties.deleteDialog.confirm")}
							</AlertDialogAction>
						</AlertDialogFooter>
					</AlertDialogContent>
				</AlertDialog>
									...
									<DropdownMenuContent>
										...
										<DropdownMenuCheckboxItem
											...
											onCheckedChange={(checked) => {
												if (checked) handleStatusChange(property._id, "available");
											}}
											onClick={(e) => {
												e.stopPropagation();
											}}
										>
											{t("my-properties.status.available")}
										</DropdownMenuCheckboxItem>
										<DropdownMenuCheckboxItem
											checked={property.status === "sold"}
											onCheckedChange={(checked) => {
												if (checked) handleStatusChange(property._id, "sold");
											}}
											onClick={(e) => {
												e.stopPropagation();
											}}
										>
											{t("my-properties.status.sold")}
										</DropdownMenuCheckboxItem>
										<DropdownMenuCheckboxItem
											checked={property.status === "pending"}
											onCheckedChange={(checked) => {
												if (checked) handleStatusChange(property._id, "pending");
											}}
											onClick={(e) => {
												e.stopPropagation();
											}}
										>
											{t("my-properties.status.pending")}
										</DropdownMenuCheckboxItem>
										<DropdownMenuCheckboxItem
											checked={property.status === "rented"}
											onCheckedChange={(checked) => {
												if (checked) handleStatusChange(property._id, "rented");
											}}
											onClick={(e) => {
												e.stopPropagation();
											}}
										>
											{t("my-properties.status.rented")}
										</DropdownMenuCheckboxItem>
										<DropdownMenuCheckboxItem
											checked={property.status === "off-market"}
											onCheckedChange={(checked) => {
												if (checked) handleStatusChange(property._id, "off-market");
											}}
											onClick={(e) => {
												e.stopPropagation();
											}}
										>
											{t("my-properties.status.off-market")}
										</DropdownMenuCheckboxItem>
