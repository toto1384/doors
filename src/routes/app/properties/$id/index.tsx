...
export const ScheduleViewComponent = ({ id, property }: { id: string; property: Partial<PropertyObject> }) => {
	...
	const {
		...
		queryFn: () =>
			trpcClient.appointments.getSellerAvailability.query({ propertyId: id, month: selectedMonth, year: selectedYear }),
	});
	...
	const handleScheduleViewing = (date: Date, time: string) => {
		...
		return scheduleViewingMutation
			.mutateAsync({
				propertyId: id,
				date: date,
				startTime: time,
	...
	const { attach, check } = useCustomer();
...
function PropertyDetailRoute() {
	...
	const setFavoriteMutation = useMutation(
		trpc.properties.setPropertyFavorite.mutationOptions({
			onSuccess: () => {
				setIsSaved(!isSaved);
			},
		}),
			...
			>
				<svg
					...
					style={{
						...
						backdropFilter: "blur(100px) grayscale(1) contrast(100)",
						clipPath: "url(#back-arrow-clip)",
				...
				>
					<defs>
						<clipPath id="back-arrow-clip">
							<path
								d="M3.9334 9H16.1084V7H3.9334L9.5334 1.4L8.1084 0L0.108398 8L8.1084 16L9.5334 14.6L3.9334 9Z"
								fill="#0B0014"
							/>
						</clipPath>
					</defs>
...
function PropertyImageGallery({ property }: { property: PropertyObject }) {
	...
	const nonSelected = property.imageUrls.filter((i, ind) => {
		return ind !== selectedImageIndex;
	});
...
export function DescriptionSection({
	...
	return (
		<div className="px-4 mb-6">
			...
			<p
				ref={textRef}
				className={`text-[#919EAB] whitespace-pre-line text-sm leading-relaxed ${
		...
		<div className="px-4 mb-6">
			...
			{property.location && (
				<div className="bg-gray-800 rounded-lg h-64 overflow-hidden">
					<GoogleMapPreview location={property.location} height="256px" zoom={16} showMarker={true} />
				</div>
			)}
		</div>
	);
