...
export const getFavouritesProperties = createServerFn().validator((params) => z.object({ skip: z.number().optional() }).parse(params)).handler(async ({ data }) => {
    ...
    const properties = await PropertyModel.find({
        _id: { $in: favourites }
    }).skip(data.skip ?? 0).limit(6).lean() as PropertyObject[];
    ...
    return (
        <div className="md:mx-5 border border-[#1C252E] rounded-xl min-h-screen text-white">
            ...
            <div className="flex items-center justify-between mb-8 md:border-b rounded-b-lg">
                {session && <div className="flex p-6 flex-row items-center ">
                    ...
                    <div className="bg-[#2B1C37]/20 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M1.4875 9.38755L7.13438 14.6594C7.36875 14.8782 7.67812 15 8 15C8.32187 15 8.63125 14.8782 8.86563 14.6594L14.5125 9.38755C15.4625 8.50317 16 7.26255 16 5.96567V5.78442C16 3.60005 14.4219 1.73755 12.2688 1.37817C10.8438 1.14067 9.39375 1.6063 8.375 2.62505L8 3.00005L7.625 2.62505C6.60625 1.6063 5.15625 1.14067 3.73125 1.37817C1.57812 1.73755 0 3.60005 0 5.78442V5.96567C0 7.26255 0.5375 8.50317 1.4875 9.38755Z" fill="#919EAB" />
                            </svg>
                    ...
                    <div className="bg-[#2B1C37]/20 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <svg width="14" height="14" viewBox="0 0 15 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M3.66602 1V2H2.16602C1.33789 2 0.666016 2.67188 0.666016 3.5V5H14.666V3.5C14.666 2.67188 13.9941 2 13.166 2H11.666V1C11.666 0.446875 11.2191 0 10.666 0C10.1129 0 9.66602 0.446875 9.66602 1V2H5.66602V1C5.66602 0.446875 5.21914 0 4.66602 0C4.11289 0 3.66602 0.446875 3.66602 1ZM14.666 6H0.666016V14.5C0.666016 15.3281 1.33789 16 2.16602 16H13.166C13.9941 16 14.666 15.3281 14.666 14.5V6Z" fill="#919EAB" />
                            </svg>
            ...
            <div className="space-y-4 px-6 grid grid-cols-2 md:grid-cols-3 gap-6">
                {favouriteProperties.map((property) => (<PropertyCard match={'hide'} key={property._id} property={property} />))}
                {!favouriteProperties.length && <div className="text-center col-span-2 md:col-span-3 h-full py-8 flex flex-col gap-2 md:flex-row items-center justify-center">
                ...
                {favouriteProperties.length < count && <button className="bg-gradient-to-br from-[#4C7CED] to-[#7B31DC] cursor-pointer hover:bg-[#6A2BC4] cols-span-2 md:col-span-3 mb-3 w-fit mx-auto px-4 py-2 text-white text-sm rounded-[6px] flex items-center justify-center gap-2" onClick={async () => {
