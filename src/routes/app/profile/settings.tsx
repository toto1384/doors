...
function ProfileSettings() {
	...
	const accounts = useQuery({
		queryKey: ["better_auth_account"],
		queryFn: async () => await authClient.listAccounts({ fetchOptions: { cache: "no-cache" } }),
	});
	...
	return (
		<div className="md:mx-5 md:border rounded-lg min-h-screen text-white relative">
			{/* Header */}
			{session && (
				<div className="flex p-6 mb-3 flex-row items-center md:border-b rounded-b-lg">
			...
			<div className="space-y-8 max-w-3xl mx-auto">
				...
				{accounts.data?.data?.find((j) => j.providerId == "credential") && (
					<div className="md:bg-[#2B1C37]/20 rounded-lg px-6 md:py-6">
						<div className="flex items-center gap-2 mb-6">
				...
				<div className="md:bg-[#2B1C37]/20 rounded-lg px-6 md:py-6">
					...
					<form
						...
						onSubmit={notificationForm.handleSubmit(handleNotificationSave)}
						className="space-y-6"
					>
				...
				{(passwordForm.formState.isDirty || accountForm.formState.isDirty || notificationForm.formState.isDirty) && (
					<div className="sticky bottom-20 md:bottom-0 px-3 md:px-0 pb-8 bg-[#0E0218] md:bg-[#0B0014] text-white">
