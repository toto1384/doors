import { File, Loader2Icon, X } from "lucide-react";
			...
			{isUploading && (
				<div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 rounded-md">
					<span className="text-white text-xs sm:text-sm">
						<Loader2Icon className="w-4 h-4 animate-spin" /> {progress}%
					</span>
				</div>
			)}
...
export const MultiImageUpload: React.FC<{
	...
	useEffect(() => {
		...
		if (isControlled && JSON.stringify(fileUrls) !== JSON.stringify(prevValueRef.current)) {
			onChange(fileUrls);
	...
	const handleUpload = useCallback(
		(filesList: FileList) => {
			...
			if (fileArray.length === 0 && maxImages) {
				toast.error(`Maximum of ${maxImages} files allowed`);
			...
			uploadFiles(fileArray).then((i) =>
				setFiles((prev) => prev.map((f) => (newFiles.find((j) => j.id == f.id) ? { ...f, isUploading: false } : f))),
			);
		},
	...
	const handleDeleteImage = useCallback(
		(id: string) => {
			setFiles((prev) => prev.map((f) => (f.id === id ? { ...f, isDeleting: true } : f)));
