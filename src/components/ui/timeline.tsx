...
const TimelineItem = React.forwardRef<HTMLLIElement, TimelineItemProps>(
    (
        ...
        const content = (
            <div
                className={`grid ${hideDate ? 'grid-cols-[auto_1fr]' : 'grid-cols-[auto_auto_1fr]'} gap-4 items-start`}
                {...(status === 'in-progress' ? { 'aria-current': 'step' } : {})}
            >
                {/* Date */}
                {!hideDate && <div className="flex flex-col justify-start pt-2.5">
                    <TimelineTime className="text-right pr-1">{date}</TimelineTime>
                </div>}

                {/* Timeline dot and connector */}
                <div className="flex flex-col items-center">
                    <div className="relative z-10">
                        {icon ? <TimelineIcon icon={icon} color={iconColor as any} status={status} iconSize={iconsize} /> : <div className="h-10 w-10 rounded-full bg-muted flex flex-row items-center justify-center" >{date}</div>}
                    </div>
                    {showConnector && (
                ...
                {/* Content */}
                <TimelineContent>
                    <TimelineHeader className={description ? '' : 'mt-2'}>
