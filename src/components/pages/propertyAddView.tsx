...
import { usePopoversOpenStore, usePropertyAddStore } from "@/src/routes/__root";
...
export const PropertyAddView = ({ demoVersion }: { demoVersion?: boolean }) => {
	...
	const checkedSteps = [
		{
			label: `${t("fields.propertyType")}${partialProperty.propertyType ? `: ${partialProperty.propertyType}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.propertyType,
		},
		{
			label: `${t("fields.location")}${partialProperty.location ? `: ${partialProperty.location?.city}, ${partialProperty.location?.state}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.location?.country,
		},
		{
			label: `${t("fields.numberOfRooms")}${partialProperty.numberOfRooms ? `: ${partialProperty.numberOfRooms}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.numberOfRooms,
		},
		{
			label: `${t("fields.surfaceArea")}${partialProperty.surfaceArea ? `: ${partialProperty.surfaceArea}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.surfaceArea,
		},
		{
			label: `${t("fields.floor")}${partialProperty.floor ? `: ${partialProperty.floor}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.floor,
		},
		{
			label: `${t("fields.buildingYear")}${partialProperty.buildingYear ? `: ${partialProperty.buildingYear}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.buildingYear,
		},
		{
			label: `${t("fields.furnished")}${partialProperty.furnished ? `: ${partialProperty.furnished}` : `: ${t("status.waiting")}`}`,
			isChecked: partialProperty?.furnished !== undefined,
		},
		{
			label: `${t("fields.heating")}${partialProperty.heating ? `: ${partialProperty.heating}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.heating,
		},
		{
			label: `${t("fields.facilities")}${partialProperty.features ? `: ${partialProperty.features?.join(", ")}` : `: ${t("status.waiting")}`}`,
			isChecked: (partialProperty?.features?.length ?? 0) > 0,
		},
		{
			label: `${t("fields.price")}${partialProperty.price ? `: ${partialProperty.price?.value} ${partialProperty.price?.currency}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.price?.value,
		},
		{
			label: `${t("fields.imageUpload")}${partialProperty.imageUrls ? `: ${partialProperty.imageUrls?.length}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.imageUrls?.length,
		},
		{
			label: `${t("fields.title")}${partialProperty.title ? `: ${partialProperty.title}` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.title,
		},
		{
			label: `${t("fields.description")}${partialProperty.description ? `: ${partialProperty.description?.substring(0, 10)}...` : `: ${t("status.waiting")}`}`,
			isChecked: !!partialProperty?.description,
		},
	];
	...
	const { setProgressBar, setAiChatbotOpen } = usePopoversOpenStore(
		useShallow((state) => ({ setProgressBar: state.setProgressBar, setAiChatbotOpen: state.setAiChatbotOpen })),
	);
		...
		<>
			<div className="grid grid-cols-1 md:grid-cols-3 gap-3 ">
				...
				{postedStatus && (
					<>
						{!demoVersion && (
							<div className="text-base font-medium text-white mb-4 md:col-span-3">
								{postedStatus.success ? (
									<div className="flex flex-col items-center">
										<CheckCircle className="text-green-400 w-5 h-5 flex-shrink-0" />
										<p> Property saved successfully </p>

										<button
											onClick={() => router.navigate({ to: "/app/my-properties" })}
											className="ml-2 text-xs text-white hover:underline bg-gradient-to-br from-[#4C7CED] to-[#7B31DC] hover:bg-[#6A2BC4] px-2 py-1 rounded-[6px]"
										>
											Go to my properties
										</button>
									</div>
						...
						{demoVersion && (
							<div className="text-base font-medium text-white mb-4 flex flex-col mx-auto items-center md:col-span-3">
								<CheckCircle className="text-green-400 w-5 h-5 flex-shrink-0" />
								<p> Property saved successfully. Log in to post your property. </p>
								<button
									onClick={() => router.navigate({ to: "/auth/$path", params: { path: "sign-in" } })}
									className="mt-2 text-xs text-white hover:underline bg-gradient-to-br from-[#4C7CED] to-[#7B31DC] hover:bg-[#6A2BC4] px-2 py-1 rounded-[6px]"
								>
									Login
								</button>
							</div>
				...
				{propertyType === "edit" && !postedStatus && (
					<div className="md:col-span-2">
									...
									<div className="mb-3">
										<div className="flex justify-between items-center mb-2">
											<span className="text-sm text-gray-400">
												{completedSteps}/{totalSteps} {t("detailsCompleted")}
											</span>
										</div>
						...
						{!!titlesAndDescriptions.length && (
							...
							>
									...
									render={({ field, fieldState }) => (
										...
										>
											{titlesAndDescriptions.map((item, index) => (
												<div
													key={index}
													className="flex items-center space-x-2 relative rounded-lg border hover:bg-gray-100/20 cursor-pointer"
												>
													<RadioGroupItem
														value={item.title}
													...
													<Label
														...
														className="whitespace-pre-line w-full h-full p-3"
													>
														{item.title}
													</Label>
												</div>
										...
										>
											{titlesAndDescriptions.map((item, index) => (
												...
												>
													...
													/>
													<Label
														htmlFor={`description-${index.toString()}`}
														className="whitespace-pre-line w-full h-full p-3"
													>
														{item.description}
													</Label>
								...
								<button
									...
									className="w-full mt-2 bg-gradient-to-br from-[#4C7CED] to-[#7B31DC] hover:bg-[#6A2BC4] text-white text-xs px-4 py-2 rounded-[6px]"
								>
									Done
								</button>
							</form>
						...
						{propertyType === "edit" && !titlesAndDescriptions.length && (
							<div className="flex flex-col w-full pt-4 pb-5 px-6 space-y-2">
								{!postedStatus && completedSteps === totalSteps && (
									<h3 className="text-base font-medium text-white mb-4"> Property details filled successfully</h3>
								)}
								{completedSteps !== totalSteps && (
									<>
