...
export function PropertiesView({ propertiesReceived, searchParams, demoVersion }: { propertiesReceived?: { properties: (PropertyObject & { matchScore: number })[], count: number, totalFilterCount: number }, searchParams: PropertyFiltersObject, demoVersion?: boolean }) {
    ...
    const {
        ...
        getNextPageParam: (lastpage, _allPages, lastPageParam) => {
            if ((lastpage?.properties.length ?? 0) === 0) { return undefined; }
    ...
    return status === 'error' ? (<span>Error: {error.message}</span>) : (
        <>
            {/* Properties Grid */}
