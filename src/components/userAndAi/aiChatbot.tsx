...
export function ElevenLabsChatBotDemo<T extends boolean>({
	conversationToken,
	user,
	demoVersion,
	userType,
	...
	const { partialProperty, setPartialProperty } = usePropertyAddStore(
		useShallow((state) => ({
	...
	const { setSendUpdate, setStartConversation, setEndConversation } = usePropertyFilterStore(
		useShallow((state) => ({
	...
	const postPropertyFunctions = useSetPropertyFunctions({
		setMessages,
		updateGhostProperty: (p) => setPartialProperty({ ...partialProperty, ...p }),
		demoVersion,
	});
	...
	const firstMessage =
		userTypeVar == undefined
			? localeVar == "ro"
				? "Doresti sa cumperi sau sa vinzi o proprietate"
			...
			: userTypeVar === "buyer"
				...
				: localeVar == "ro"
					...
					: "I'm here to help you sell your property. Are we talking about an apartment or a house?";
	...
	const dynamicVariables = {
		...
		hasAllPreferencesSet: user
			? !!(
					(user.preferences?.propertyType?.length ?? 0) > 0 &&
					(user.preferences?.budget?.min || user.preferences?.budget?.max) &&
					user.preferences?.location?.fullLocationName &&
					(user.preferences?.numberOfRooms?.length ?? 0) > 0 &&
					(user.preferences?.facilities?.length ?? 0) > 0
				)
			: false,
	...
	return (
		<IsConnectedContext.Provider value={conversation.status === "connected"}>
			<ChatComponent
				...
				sendMessage={(message) => {
					...
					return setMessages((prev) => [...prev, { message: message.text, source: "user", id: nanoid() }]);
				}}
			/>
