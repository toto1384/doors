...
export const ChatComponent = ({
	...
	const { progressBar } = usePopoversOpenStore(
		useShallow((state) => ({
			...
			<div className="flex flex-col h-full ">
				<div className="flex flex-row items-start justify-between border-b border-[#404040 rounded-b-lg">
					<div className="flex flex-row items-center px-3 pb-5 gap-2 ">
						<img
							...
							className="w-[35px] h-[35px] object-contain bg-[#525252]/10 rounded-full p-2 object-center"
						/>
					...
					<div className="p-2 md:hidden" onClick={() => setAiChatbotOpen(!aiChatbotOpen)}>
						<ChevronDown className="w-6 h-6" />
					</div>
				</div>

				{progressBar != undefined && (
					<div className="md:hidden mt-1 px-2">
						<div className="flex justify-between items-center mb-2">
							<span className="text-sm text-gray-400">
								{progressBar.checkedSteps}/{progressBar.totalSteps} {"Details Completed"}
							</span>
						</div>
				...
				<Conversation className="h-full no-scrollbar">
					<ConversationContent className="bg-transparent p-0 min-h-full flex flex-col ">
						...
						{agentState === "stopped" && (
							<div className="flex flex-col min-h-full grow-1 flex-1 items-center justify-center">
								{demoVersion && (
									<div className="text-center p-6 bg-[#241540] rounded-lg mx-4">
										{userType === "buyer" && (
											<div className="space-y-4">
												<p className="text-white text-lg">{t("ai-chatbot.demo.buyer.message")}</p>
												<Link
													to="/auth/$path"
													params={{ path: "sign-in" }}
										...
										{userType === "seller" && (
											<div className="space-y-4">
												<p className="text-white text-lg">{t("ai-chatbot.demo.seller.message")}</p>
												<Link
													to="/auth/$path"
													params={{ path: "sign-in" }}
													className="inline-block bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-3 rounded-md font-semibold hover:from-blue-600 hover:to-purple-700 transition-all"
										...
										{userType === undefined && (
											<div className="space-y-4">
												<p className="text-white text-lg">{t("ai-chatbot.demo.undefined.message")}</p>
												<Link
													to="/auth/$path"
													params={{ path: "sign-in" }}
													className="inline-block bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-3 rounded-md font-semibold hover:from-blue-600 hover:to-purple-700 transition-all"
								...
								{!demoVersion && (
									<div className="text-center p-6 bg-[#241540] rounded-lg mx-4">
										{userType === "buyer" && (
											<p className="text-white text-lg">{t("ai-chatbot.production.buyer.message")}</p>
										)}

										{userType === "seller" && (
											<p className="text-white text-lg">{t("ai-chatbot.production.seller.message")}</p>
										)}

										{userType === undefined && (
											<p className="text-white text-lg">{t("ai-chatbot.production.undefined.message")}</p>
										)}
									</div>
								)}
							</div>
													...
													{message.source === "ai" &&
														...
														i === messages.length - 1 && (
															<Actions className="mt-2">
																...
																<Action
																	onClick={() => navigator.clipboard.writeText(message.message as string)}
																	label="Copy"
																>
				...
				<div className="px-3 w-full flex">
					<PromptInput
						...
						className={`mt-4 px-2 ${status != "connected" && "bg-transparent border-0"}`}
						globalDrop
						multiple
					>
						{status === "connected" && (
							<>
                                ...
                                <PromptInputActionMenuContent>
                                    <PromptInputActionAddAttachments />
										...
										<PromptInputButton onClick={() => endConversation()} variant={"destructive"}>
						...
						{status === "disconnected" && (
							<>
								<PromptInputButton
									...
									variant={"default"}
									className="w-full cursor-pointer text-green-900 px-7 py-3 font-semibold rounded-md"
								>
									<span className="">
										{demoVersion
											? t("ai-chatbot.startDemo")
											: userType == "buyer"
												? t("ai-chatbot.searchProperty")
												: t("ai-chatbot.listProperty")}
									</span>
									<Play size={16} />
								</PromptInputButton>
							</>
