...
export function UserTypeSwitch({ extraPadding }: { extraPadding?: boolean }) {
	...
	return (
		<div className="rounded-[5px] bg-[#32215A] w-fit flex flex-row items-center p-0.5">
			<button
				className={
					(gUserType === "buyer" ? "bg-gradient-to-br from-[#4C7CED] to-[#7B31DC]" : "") +
					` text-white text-xs ${extraPadding ? "px-5" : "px-2.5"} py-[6px] rounded hover:from-blue-600 hover:to-purple-700 transition-all`
				}
			...
			<button
				className={
					(gUserType === "seller" ? "bg-gradient-to-br from-[#4C7CED] to-[#7B31DC]" : "") +
					` text-white text-xs ${extraPadding ? "px-5" : "px-2.5"} py-[6px] rounded hover:from-blue-600 hover:to-purple-700 transition-all`
				}
