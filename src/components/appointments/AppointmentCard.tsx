...
import { AlertCircle, Calendar as CalendarIcon, Clock, ExternalLink, MessageCircle, Star } from "lucide-react";
			...
			case "pending":
				return (
					<Badge variant="secondary" className="bg-yellow-500/20 text-yellow-400">
						{t(`${translationKey}.statusPending`)}
					</Badge>
				);
			case "confirmed":
				return (
					<Badge variant="default" className="bg-green-500/20 text-green-400">
						{t(`${translationKey}.statusConfirmed`)}
					</Badge>
				);
			...
			case "cancelled-by-seller":
				return (
					<Badge variant="destructive" className="bg-red-500/20 text-red-400">
						{t(`${translationKey}.statusCancelled`)}
					</Badge>
				);
			case "completed":
				return (
					<Badge variant="outline" className="bg-blue-500/20 text-blue-400">
						{t(`${translationKey}.statusCompleted`)}
					</Badge>
				);
			default:
				...
				<div className="flex flex-col md:flex-row items-start gap-4">
					...
					<div className="flex-1 min-w-0">
						<div className="flex items-start justify-between mb-3">
							<div>
								<h3 className="font-semibold text-white text-base mb-1">{appointment.property?.title || "Property"}</h3>
						...
						<div className="flex gap-2">
							...
							{userRole === "buyer" && (
								<Link to="/app/properties/$id" params={{ id: appointment.propertyId }} className="inline-flex">
