        ...
        <>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Avatar className="h-8 w-8 rounded-full flex items-center justify-center">
                            <AvatarImage className="rounded-full aspect-square object-cover" src={session?.user?.image || ""} alt={session?.user?.name || ""} />
                            <AvatarFallback className="rounded-full bg-[#130826] border text-base/1 w-8 h-8 flex flex-col items-center justify-center">{session?.user?.name?.charAt(0) || session?.user?.email?.charAt(0) || "U"}</AvatarFallback>
                        </Avatar>
                    </DropdownMenuTrigger>
            ...
            <div className="md:hidden">
                <Avatar onClick={() => setProfileMenuOpen(true)} className="h-8 w-8 flex items-center justify-center">
                    <AvatarImage className="rounded-full aspect-square object-cover" src={session?.user?.image || ""} alt={session?.user?.name || ""} />
                    <AvatarFallback className="rounded-full">{session?.user?.name?.charAt(0) || session?.user?.email?.charAt(0) || "U"}</AvatarFallback>
                </Avatar>
            </div>
