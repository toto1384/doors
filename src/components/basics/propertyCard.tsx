...
export const PropertyCard = ({ property, match, matchRight, moreComponent, disableLink, demoVersion }: {
    ...
    const propertyContent = <>
        ...
        <div className="flex flex-row justify-between" >
            ...
            <div className='flex flex-col'>
                <div className={`flex items-center gap-1 mt-2 ${demoVersion ? 'text-[8px]' : `text-[10px]`} text-[#a3a3a3]`}>
                    {[
                        ...(property.numberOfRooms ? [{ icon: <BedIcon className={demoVersion ? "w-2 h-2" : "w-3 h-3"} color="#ffffff" />, text: `${property.numberOfRooms}` }] : []),
                        ...(property.numberOfBathrooms ? [{ icon: <BathIcon className={demoVersion ? 'w-2 h-2' : 'w-3 h-3'} color="#ffffff" />, text: `${property.numberOfBathrooms}` }] : []),
                        ...(property.surfaceArea ? [{ icon: <SurfaceAreaIcon className={demoVersion ? 'w-2 h-2' : 'w-3 h-3'} color="#ffffff" />, text: `${property.surfaceArea}m²` }] : []),
        ...
        <h3 className={` ${demoVersion ? 'text-[13px]' : 'text-sm md:text-lg md:mb-2'} ${demoVersion ? "line-clamp-2" : "line-clamp-3"} font-normal text-start `}>{property.title}</h3>

        <div className={`flex flex-row text-[#637381] ${demoVersion ? 'text-[8px]' : 'text-xs md:text-md md:mt-2'}  text-start items-center gap-1 `}>
