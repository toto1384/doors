...
export function DropDownOrBottomSheet(props: DropdownProps | DropdownPropsAlternative) {
	...
	if (size.llg) {
		return (
			<div className={`relative overflow-y-hidden ${props.dropdownClassName}`}>
				<div
					className={`cursor-pointer left-0 `}
					data-cy={`dropdown-${props.dataCy ?? props.id}`}
					{...(props.onMouseDownAlternative ? { onMouseDown: onClick } : { onClick })}
				>
				...
				{toggled && (
					<div
						className="backdrop fixed inset-0 z-40 overflow-y-hidden h-screen w-screen"
						{...(props.onMouseDownAlternative ? { onMouseDown: onClick } : { onClick })}
					></div>
				)}
				<div
					className={`fixed left-0 right-0 transition-all overflow-scroll bg-white mt-20 z-50 ${props.topBs ? "max-h-[60vh] min-h-[60vh]" : "max-h-[85vh]"}  rounded-md ${toggled ? "bottom-0 mt-32" : "bottom-[-400vh]"}`}
				>
					{/* <div className=""></div> */}
					<div className="flex px-3 pt-3 flex-row items-center">
						<ArrowLeft
							{...(props.onMouseDownAlternative ? { onMouseDown: onClick } : { onClick })}
							className="h-10 w-10 icon-button p-1"
						/>
					</div>
...
export function DropDownOrBottomSheetAlt(
	props: (DropdownProps | DropdownPropsAlternative) & { additionalMobileHeader?: ReactNode },
) {
	const size = useSize(true);
	...
	if (size.llg) {
		return (
			<div className={`relative overflow-y-hidden ${props.dropdownClassName}`}>
				...
				<BottomSheet
					open={toggled}
					className="w-full bg-white absolute z-[50]"
