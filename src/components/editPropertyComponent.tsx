...
export function EditPropertyComponent<T extends Partial<PropertyObject> | PropertyObject>({
	property,
	onPropertyChange,
	onSave,
	saveWithoutEdit,
	...
	property: T;
	onPropertyChange: (property: T) => void;
	onSave: () => Promise<void>;
	saveWithoutEdit?: boolean;
	landingPage?: boolean;
}) {
	...
	return (
		<div className="flex flex-col pb-15">
			...
			{editingStates.titlePriceRoomsSqFeet ? (
				<EditPropertyTitlePriceRoomsSqFeet
					property={property}
...
function EditPropertyFeatures({
	property,
	onPropertyChange,
}: {
	property: Partial<PropertyObject>;
	onPropertyChange: (property: Partial<PropertyObject>) => void;
}) {
	...
	return (
		<div className="mx-4">
			<FacilitiesSelector
				className="mt-2"
				facilities={initialFeatures as any}
				setFacilities={(value) => setInitialFeatures(value)}
			/>

			<Button
				className="w-full mt-2 bg-gradient-to-br from-[#4C7CED] to-[#7B31DC] text-white text-xs px-4 py-2 rounded-[6px]"
				onClick={() => onPropertyChange({ ...property, features: initialFeatures })}
			>
				Done
			</Button>
		</div>
...
function EditPropertyLocation({
	...
	return (
		<div className="mx-4">
			...
			<Button
				className="w-full mt-2 bg-gradient-to-br from-[#4C7CED] to-[#7B31DC] text-white text-xs px-4 py-2 rounded-[6px]"
				onClick={() => onPropertyChange({ ...property, location: initialLocation })}
			>
				Done
			</Button>
		</div>
...
function EditPropertyDescription({
	...
	return (
		<form
			...
			onSubmit={handleSubmit((object) => onPropertyChange({ ...property, description: object.description }))}
		>
			<Textarea
				id="description"
			...
			<Button type="submit" className="rounded-lg p-2 text-white bg-gradient-to-br from-[#4C7CED] to-[#7B31DC]">
				Done
			</Button>
		</form>
	);
...
function EditPropertyTitlePriceRoomsSqFeet({
	property,
	onPropertyChange,
}: {
	property: Partial<PropertyObject>;
		...
		>
			...
			<div className="flex flex-row items-start gap-2 my-2">
				<div className="flex flex-col">
					...
					/>
					{errors.price?.currency && <p className="text-red-400 text-sm mt-1">{errors.price.currency.message}</p>}

					{errors.price?.value && <p className="text-red-400 text-sm mt-1">{errors.price.value.message}</p>}
				</div>
			...
			<>
				...
				/>
				{errors.numberOfRooms && <p className="text-red-400 text-sm mt-1">{errors.numberOfRooms.message}</p>}
			</>
			...
			<>
				...
				/>
				{errors.numberOfBathrooms && <p className="text-red-400 text-sm mt-1">{errors.numberOfBathrooms.message}</p>}
			</>
			...
			<>
				...
				/>
				{errors.surfaceArea && <p className="text-red-400 text-sm mt-1">{errors.surfaceArea.message}</p>}
			</>
