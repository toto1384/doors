...
import { CalendarIcon, Clock, Loader2Icon, MapPin, User } from "lucide-react";
						...
						<CardContent className="p-4">
							<div className="flex items-start gap-3">
								{property.imageUrls && property.imageUrls[0] && (
									<img src={property.imageUrls[0]} alt={property.title} className="w-16 h-16 rounded-lg object-cover" />
								)}
								<div>
									...
									{property.price && <p className="text-lg font-bold ">€{property.price.value?.toLocaleString()}</p>}
								</div>
							</div>
						</CardContent>
					</Card>
									...
									<CardContent className="p-4">
										...
										<div className="space-y-2 text-sm">
											...
											<div className="flex items-center gap-2">
												...
												{selectedTime} -{" "}
												{format(new Date(parse(selectedTime, "HH:mm", new Date()).getTime() + 60 * 60 * 1000), "HH:mm")}
											</div>
											<div className="flex items-center gap-2">
