...
import { useChooseActions } from "@/src/components/userAndAi/useChooseActionsAiHook";
	...
	tools.tools.map((tool) => {
		...
		if (tool.toolConfig.type === "client") {
			return {
				...
				parameters: tool.toolConfig.parameters?.properties
					? Object.entries(tool.toolConfig.parameters?.properties).map(([k, v]) => ({ id: k, ...v }))
					: undefined,
			};
		}
...
export const useConversation =
	...
	({
		...
		const [conversationEvents, setConversationEvents] = React.useState<
			{ onConnect: Function; onDisconnect: Function; onMessage: Function } | undefined
		>();
